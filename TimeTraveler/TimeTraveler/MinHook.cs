...
{
    public class MinHook : IDisposable
    {
        private static Lazy<IntPtr> _module = new Lazy<IntPtr>(HookNativeProcs, true);
        ...
        private static IntPtr HookNativeProcs()
        {
            ...
            if (path == null)
                throw new TimeTravelerException("Cannot determine native MinHook dll path. Process is running " + (IntPtr.Size == 8 ? "64" : "32") + "-bit.");

            NativeDllPath = path;
            var module = LoadLibrary(path);
            if (module == IntPtr.Zero)
                throw new TimeTravelerException("Cannot load native MinHook dll from path '" + path + "'. Process is running " + (IntPtr.Size == 8 ? "64" : "32") + "-bit.", new Win32Exception(Marshal.GetLastWin32Error()));
        ...
        private static T LoadProc<T>(IntPtr module, string name = null)
        {
            ...
            if (address == IntPtr.Zero)
                throw new TimeTravelerException("Cannot load library function '" + name + "' from '" + NativeDllPath + "'. Please make sure MinHook is the latest one.", new Win32Exception(Marshal.GetLastWin32Error()));
